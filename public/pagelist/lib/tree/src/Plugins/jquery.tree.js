...
(function($) {
    ...
    $.fn.treeview = function(settings) {
        var dfop =
            {
                 ...
                 */
                cbiconpath: "./lib/tree/css/images/icons/",
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            //a
            ht.push("<a hideFocus class='bbit-tree-node-anchor' tabIndex=1 href='#'>");
        ...
        function nodeclick(e) {
            ...
            }else if(et.tagName == "SPAN" && $(et.parentElement.parentElement).find(".bbit-tree-elbow-minus,.bbit-tree-elbow-plus,.bbit-tree-elbow-end-plus,.bbit-tree-elbow-end-minus").length>0){
                var etitem=$(et).parent().parent().find(".bbit-tree-ec-icon");
                if ($(etitem).hasClass("bbit-tree-elbow-plus") || $(etitem).hasClass("bbit-tree-elbow-end-plus")) {
                    var ul = $(this).next(); //"bbit-tree-node-ct"
                    if (ul.hasClass("bbit-tree-node-ct")) {
                        ul.show();
                    }
                    ...
                    if ($(etitem).hasClass("bbit-tree-elbow-plus")) {
                        $(etitem).swapClass("bbit-tree-elbow-plus", "bbit-tree-elbow-minus");
                    }
                    else {
                        $(etitem).swapClass("bbit-tree-elbow-end-plus", "bbit-tree-elbow-end-minus");
                    }
                    $(this).swapClass("bbit-tree-node-collapsed", "bbit-tree-node-expanded");
                }
                //if expended, collapse it
                else if ($(etitem).hasClass("bbit-tree-elbow-minus") || $(etitem).hasClass("bbit-tree-elbow-end-minus")) {
                    $(this).next().hide();
                    if ($(etitem).hasClass("bbit-tree-elbow-minus")) {
                        $(etitem).swapClass("bbit-tree-elbow-minus", "bbit-tree-elbow-plus");
                    }
                    else {
                        $(etitem).swapClass("bbit-tree-elbow-end-minus", "bbit-tree-elbow-end-plus");
                    }
                    $(this).swapClass("bbit-tree-node-expanded", "bbit-tree-node-collapsed");
                }
                else if ($(etitem).hasClass("bbit-tree-node-cb")) // click on checkbox
                {
                    var s = item.checkstate != 1 ? 1 : 0;
                    var r = true;
                    if (dfop.oncheckboxclick) {
                        r = dfop.oncheckboxclick.call(etitem, item, s);
                    }
                    if (r != false) {
                        if (dfop.cascadecheck) {
                            cascade(check, item, s);
