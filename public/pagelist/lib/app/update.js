function update(y,m) {
    ...
    var td=((monthlyExports[y*15+m] - monthlyImports[y*15+m]))*-1
    var fs=(15 + (String(td).length)*2);
    ...
    dGroup.transition()
        ...
        .delay(delay)
        .text(td)
        .attr("transform", "translate(" + (outerRadius - (td.length * fs/2)/2) + ","  + (outerRadius + 30) +")")
    ...
    iText.enter()
        ...
        .text(function(d) { return  (d.index+1)  + ". " + d.label+"产能量"; })
        .on("mouseover", function (d) { node_onMouseOver(d); })
        .on("mouseout", function (d) {node_onMouseOut(d); });
...
function updateMonths(y,m) {
    ...
    monthEnter.append("text")
        .attr("transform",function (d,i) { return "translate (" + String(i*monthOffset-10) + ",0)"; })
        .text(function(d,i) { return monthsMap[i % 15]; })
    ...
    monthEnter.append("text")
        ...
        .text(function(d,i) {
            if ((i==0) || (i % 15==0)) {
                return String(months[i].month.substr(5))
                //return String(baseYear + Math.floor(i/15));
            }
            else
                return String(months[i].month.substr(5))
                //return "";
