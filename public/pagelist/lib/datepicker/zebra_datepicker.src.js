                ...
                if (!update) {
                    ...
                    clickables.bind('click', function(e) {
                      //  $(this).css("background","black");
                        if (this.id.indexOf("DatePicker") > -1 && this.type == "text") {
                            this.focus();    
                            this.select();
                               $(this).unbind("blur").blur(function () {
                                    if (!(this.value.match("^[1-9][0-9]{3}[-/][0-9]{1,2}[-/][0-9]{1,2}$"))) {
                                        if(!(this.value.match("^[1-9][0-9]{3}[-/][0-9]{1,2}$"))){
                                            $(this).css("background", "#fc8782");
                                            $(this).val("");
                                        }else{
                                            var timeTemp = $(this).val().split('-');
                                            var default_date = new Date(timeTemp[0], timeTemp[1],1, 12, 0, 0)
                                            plugin.settings.onSelect.call($element, $(this).val(), $(this).val(), default_date, $element);
                                        }
                                    }
                                    else {
                                        var timeTemp = $(this).val().split('-');
                                        var default_date = new Date(timeTemp[0], timeTemp[1], timeTemp[2], 12, 0, 0)
                                        plugin.settings.onSelect.call($element, $(this).val(), $(this).val(), default_date, $element);
                                    }
                                });
                           }
                ...
                '<div class="Zebra_DatePicker">' +
                    '<table class="dp_header">' +
                        '<tr>' +
                            ...
                            '<td class="dp_caption" style="color:#ffffff;font-weight: bold;">&#032;</td>' +
                    ...
                    '<table class="dp_footer"><tr>' +
                        '<td class="dp_today"' + (plugin.settings.show_clear_date !== false ? ' style="width:50%;color:#ffffff;font-weight: bold"' : '') + '>' + show_select_today + '</td>' +
                        '<td class="dp_clear"' + (show_select_today !== false ? ' style="width:50%;color:#ffffff;font-weight: bold"' : '') + '>' + plugin.settings.lang_clear_date + '</td>' +
                    '</tr></table>' +
                '</div>';
                ...
                if (regexp.length) {
                    ...
                    if ((segments = regexp.exec(str_date))) {
                        ...
                        var tmpdate = new Date(),
                            ...
                            english_days   = ['日','一','二','三','四','五','六'],
                            english_months = ['1月','2月','3月','4月','5月','6月','7月','8月','9月','10月','11月','12月'],
                            iterable,
